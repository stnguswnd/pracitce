...
async function getProductById2(n) {
  const res = await axios.get(`${BASE_URL}/products/${n}`);
  const data = res["data"]["reviews"];
  ...
  /* 내가 짠 코드인데 위에 강사님이 짜신 코드가 더 좋다.
  const rateScore = data.map((item) => item.rating);

  const sumRate = rateScore.reduce((acc, todo) => {
    return acc + todo;
  });
  console.log(sumRate / rateScore.length);
  */
}
//getProductById2(10);
